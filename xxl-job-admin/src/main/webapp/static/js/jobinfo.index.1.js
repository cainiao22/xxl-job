$(function() {
	                	...
	                	"render": function ( data, type, row ) {
	                		return function(){
								...
								var html = '<p id="'+ row.id +'" >'+
									'<button class="btn btn-primary btn-xs job_trigger" _type="job_trigger" type="button">执行</button>  '+
									pause_resume +
    ...
    $("#job_list").on('click', '.job_trigger',function() {
        ...
        // base data
        $("#executeModal .form input[name='id']").val( row.id );


        // show
        $('#executeModal').modal({backdrop: false, keyboard: false}).modal('show');
    });

    //执行按钮
    $("#executeModal .form").validate({
        submitHandler: function (form) {
            $.post(base_url + "/jobinfo/trigger", $("#executeModal .form").serialize(), function(data, status) {
                if (data.code == "200") {
