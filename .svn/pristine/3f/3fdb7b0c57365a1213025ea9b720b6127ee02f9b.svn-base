		...
		while(!toStop){
			...
			try {
				...
				if (triggerParam!=null) {
					...
					TriggerCallbackThread.pushCallBack(new HandleCallbackParam(triggerParam.getLogId(), ReturnT.RUNNING));
					// parse param
...
//							? (String[])(Arrays.asList(triggerParam.getExecutorParams().split(",")).toArray()) : null;
					String handlerParams=triggerParam.getExecutorParams();
					// handle job
					...
					try {
						...
						jobInfoParam.setJobId(triggerParam.getJobId());
						jobInfoParam.setJobName(triggerParam.getJobName());
						jobInfoParam.setScheduleTime(triggerParam.getScheduleTime());
						executeResult = handler.execute(jobInfoParam, handlerParams);
